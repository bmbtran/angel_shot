        ...
        NavigationView {
            VStack {
                ...
                VStack {
                    Group {
                        ...
                        .onChange(of: reportmodeViewModel.frameText) { newValue in
                            ...
                            reportmodeViewModel.isValidFrameText = newValue
                ...
                VStack {
                    ...
                    HStack {
                        TextField(StringConstants.kLbl00, text: $reportmodeViewModel.labelthreeText)
                            .font(FontScheme.kInterBold(size: getRelativeHeight(16.0)))
                            .foregroundColor(ColorConstants.Black900)
                            .padding()
                    }
            ...
            .actionSheet(isPresented: $reportmodeViewModel.isActionSheetShow, content: {
                ActionSheet(title: Text("Choose"), buttons: [
                    .default(Text("Camera"), action: {
                        if UIImagePickerController.isSourceTypeAvailable(.camera) {
                            reportmodeViewModel.imagePickerSource = .camera
                            reportmodeViewModel.isImagePickerShow = true
                        } else {
                            reportmodeViewModel.showAlert("Error", "Camera is not available!")
                        }
                    }),
                    .default(Text("Photo Library"), action: {
                        reportmodeViewModel.imagePickerSource = .photoLibrary
                        reportmodeViewModel.isImagePickerShow = true
                    }),
                    .destructive(Text("Cancel"))
                ])
            })
            .sheet(isPresented: $reportmodeViewModel.isImagePickerShow, content: {
                MediaPicker(sourceType: reportmodeViewModel.imagePickerSource,
                            mediaTypes: ["public.image"]) { image, url in
                    if let image = image {
                        reportmodeViewModel.selectedImage = image
                    }
                }
            })
            .hideNavigationBar()
